...
public class GuestBookMgr {
	...
	//GuestBook List
	public Vector<GuestBookBean> listGuestBook
	(String id, String grade){
		...
		try {
			con = pool.getConnection();
			if(grade.equals("1")) { //관리자가 로그인
				sql = "select * from tblGuestBook order by num desc";
				pstmt = con.prepareStatement(sql);
			}else if(grade.equals("0")) {//일반 로그인 : 본인 + 비밀글(X)
	...
	public Vector<CommentBean> listComment(int num){
		...
		try {
			...
			while(rs.next()) {
				...
				bean.setCnum(rs.getInt(1));//댓글 번호
				bean.setNum(rs.getInt(2));//방명록 번호
				bean.setCid(rs.getString(3));//댓글 쓴 id
				bean.setComment(rs.getString(4));//댓글 내용
				bean.setCip(rs.getString(5));//ip 주소값
				String teamDate = 
						SDF_DATE .format(rs.getDate(6));
				bean.setCregDate(teamDate);
				vlist.addElement(bean);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	//Comment All Delete : 방명록 원글 삭제시 관련된 댓글 모두 삭제
